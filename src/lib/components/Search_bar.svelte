<script>
  ...
  const handle_course_search = () => {
    const matched_courses = searched_university.courses.filter((course) =>
      course.toLowerCase().includes(search_value)
    );
...
<div class="border-2 border-gray-300 rounded-2xl w-full">
  <input
    ...
    placeholder={!!searched_university
      ? "search for a course"
